...
export default ({ config, db }) => {
	...
	api.all('/login', async (req, res) => {
		...
		try {
			...
			let userinfo = await YonghuModel.findOne({ where: { gerenzhanghao: req.query.username, mima: req.query.password } })
			if (userinfo === null) {
			...
			const token = jwt.sign(
				{
					...
					username: userinfo.dataValues.gerenzhanghao,
					role: '用户',
                    loginUserColumn: 'gerenzhanghao',
                    tableName: 'yonghu'
				},
				config.jwtSecret,
				{
			...
			userinfo.dataValues.loginUserColumn = 'gerenzhanghao'
	...
	api.all('/resetPass', async (req, res) => {
		...
		try {
			...
			await YonghuModel.update({
				...
				where: {
				  gerenzhanghao: req.query.username
				}
			})
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let where = {}
			var gerenzhanghao = req.query.gerenzhanghao
			if (gerenzhanghao) {

				if (gerenzhanghao.indexOf('%') != -1) {
					where.gerenzhanghao = {
						[Op.like]: gerenzhanghao
					}
				} else {
					where.gerenzhanghao = {
						[Op.eq]: gerenzhanghao
