<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="商品类型" prop="shangpinleixing">
          <el-input v-model="ruleForm.shangpinleixing" 
              placeholder="商品类型" clearable  :readonly="ro.shangpinleixing"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="商品类型" prop="shangpinleixing">
              <el-input v-model="ruleForm.shangpinleixing" 
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shangpinleixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shangpinleixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
